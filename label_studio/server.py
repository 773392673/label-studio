...
def _setup_environment(input_args, config):
    """Set up environment variables and initialize Django."""
    # setup logging level
...
def _start_server(input_args, config, host):
    """Launch browser and start the Label Studio server."""
...
def _dispatch_command(input_args, config, host):
    ...
    # init project
        ...
        if input_args.command != 'start' and getattr(input_args, 'project_name', None):
    ...
    # start with migrations from old projects, '.' project_name means 'label-studio start' without project name
    elif input_args.command == 'start' and getattr(input_args, 'project_name', None) and input_args.project_name != '.':
        from projects.models import Project
        ...
        if not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                ...
                config_project = _get_config(config_path)
                user = _create_user(input_args, config_project)
                label_config_path = project_path / 'config.xml'
                project = _create_project(
                    title=input_args.project_name,
                    user=user,
                    label_config=label_config_path,
                    sampling=sampling_map.get(config_project.get('sampling', 'sequential'), Project.UNIFORM),
                    description=config_project.get('description', ''),
                )
                migrate_existing_project(project_path, project, config_project)
                migrated = True
...
def main():
    ...
    host = _setup_environment(input_args, config)
    _dispatch_command(input_args, config, host)
